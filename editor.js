...
function renderPresetLibrary(){
  ...
  const filtered = stylePresets.filter(item=>(currentPresetCategory==="all"||item.category===currentPresetCategory)&&`${item.name} ${item.category} ${item.keywords}`.toLowerCase().includes(search));
...
function renderBackgroundLibrary(){
  ...
  const filtered = backgroundPresets.filter(bg=>(currentBackgroundCategory==="all"||bg.category===currentBackgroundCategory)&&`${bg.name} ${bg.category} ${bg.keywords}`.toLowerCase().includes(search));
...
function applyStylePreset(item){
  ...
  if(!selected){editor.addComponents(`<a href="#" class="main-btn" style="${styleObjToString(item.css)}">${item.name}</a>`);return;}
...
function applyFont(font){
  const selected = editor.getSelected();
  if(!selected){editor.addComponents(`<h2 style="${styleObjToString(font.css)}">${font.name}</h2>`);return;}
...
async function uploadImageToSupabase(){
  ...
  if(!input.files.length){status.textContent="Choose an image first.";return;}
  const file = input.files[0];
  const filePath = `${currentUser.id}/${Date.now()}-${file.name}`;
  status.textContent = "Uploading...";
  const { error } = await db.storage.from("site-images").upload(filePath,file);
  if(error){status.textContent="Upload failed. Check your site-images bucket.";return;}
...
function saveCurrentPageToMemory(){
  pages[activePage] = {html:editor.getHtml(),css:editor.getCss()};
}
...
function createNewPage(){
  ...
  if(pages[slug]){alert("That page already exists.");return;}
  saveCurrentPageToMemory();
  pages[slug]={html:`<section class="content-section"><h1>${formatPageName(slug)}</h1><p>Start building this page.</p></section>`,css:starterCss};
...
function createPresetPage(type){
  saveCurrentPageToMemory();
  pages[type]={html:`<section class="content-section"><h1>${formatPageName(type)}</h1><p>Start building this page.</p></section>`,css:starterCss};
  loadPageIntoEditor(type);
  renderPageSelect();
  renderPageList();
}
function deletePage(pageName){
  if(pageName==="home"){alert("Home page cannot be deleted.");return;}
...
function renderPageList(){
  ...
  Object.keys(pages).forEach(page=>{
    ...
    item.innerHTML = `<button onclick="switchPage('${page}')">${formatPageName(page)}</button>${page!=="home"?`<button class="delete-page" onclick="deletePage('${page}')">Delete</button>`:""}`;
    box.appendChild(item);
  });
}

async function loadSavedPage(){
  const { data, error } = await db.from("visual_pages").select("*").eq("user_id",currentUser.id).single();
  if(error) return null;
  return data;
}
async function savePage(){
  saveCurrentPageToMemory();
  const { error } = await db.from("visual_pages").upsert({user_id:currentUser.id,html:pages.home?.html||editor.getHtml(),css:pages.home?.css||editor.getCss(),pages:pages,active_page:activePage,status:"draft",updated_at:new Date().toISOString()},{onConflict:"user_id"});
  if(error){alert("Save failed");return;}
  alert("Saved");
}
async function publishPage(){
  saveCurrentPageToMemory();
  const { error } = await db.from("visual_pages").upsert({user_id:currentUser.id,html:pages.home?.html||editor.getHtml(),css:pages.home?.css||editor.getCss(),pages:pages,active_page:activePage,status:"published",updated_at:new Date().toISOString()},{onConflict:"user_id"});
  if(error){alert("Publish failed");return;}
  alert("Published");
}
    ...
    setTimeout(()=>{
      ...
      forms.forEach(form=>{
        ...
        if(attrs["data-needs-formspree"]==="true" && attrs.action==="FORM_ENDPOINT_HERE"){
          ...
          if(endpoint){
            form.addAttributes({action:endpoint,"data-needs-formspree":"false"});
